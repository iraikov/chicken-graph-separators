...
(module graph-separators
 ...
 (import scheme (chicken base) srfi-1 yasos digraph)



 (define (graph-separation-vertices g #!key 
                                    (slot (lambda (n) ((node-info g n))))
                                    (set-slot! (lambda (n v) ((node-info g n) v))))
   (let ((nodes (map car (nodes g)))
         (succ  (lambda (i) (succ g i)))
         (pred  (lambda (i) (pred g i))))
     (let ((to  (lambda (n) (succ n))))
